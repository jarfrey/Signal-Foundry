-- SQLite schema for the job-signal pipeline.
-- Executed statement-by-statement by Db.migrate() on every startup, so every
-- statement must be idempotent and there must be no bare SELECTs in here.
...
-- One row per scrape of one board. Gives us the history that makes
-- "more/less than normal" answerable across runs.
...
CREATE TABLE IF NOT EXISTS posting (
    ...
    -- labels applied by Labeler at ingest time; stored so stats are plain SQL
    seniority       TEXT,
    function        TEXT,
    country         TEXT,
    city            TEXT,
...
CREATE INDEX IF NOT EXISTS idx_posting_function       ON posting(company_id, function);

-- Nemotron output, cached per company so the demo does not re-bill or
-- re-wait on every page load. stats_hash lets us invalidate when the
...
CREATE TABLE IF NOT EXISTS insight (
    ...
    bullets_json TEXT,
    model        TEXT,
    source       TEXT NOT NULL DEFAULT 'nemotron',  -- 'nemotron' | 'fallback'
    stats_hash   TEXT,
    generated_at TEXT NOT NULL
);

-- Detector output (cross-company rules), kept so the API can serve it
-- without re-running detection on every request.
...
CREATE INDEX IF NOT EXISTS idx_signal_company ON signal(company_slug);
